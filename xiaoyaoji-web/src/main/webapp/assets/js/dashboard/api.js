require(['vue', 'utils', 'clipboard', 'sortable', 'xml2json', 'json2xml'], function (Vue, utils, Clipboard, sortable) {
    var _plugins_ = [], plugins = {}, main, app;
    context.plugins.forEach(function (item) {
        if (item !== 'global') {
            _plugins_.push('../../plugin?plugin=/' + item + '/view.js');
        }
        _plugins_.push('../../plugin?plugin=/' + item + '/edit.js');
    });
...
//页面全局变量
    var page = {
        ...
        reget: function (self) {
            ...
            utils.post('/project/' + context.projectId + '.json', {}, function (rs) {
                if (rs.code != 0) {
...
//
    ...
    main = function (vueObject) {
        var api = {
             ...
             * @param name  名称
             * @param type  类型
             * @param content 内容;JSON对象
             */
            submit: function (name, type, content) {
            ...
            methods: {
                ...
                apiClick: function (item, folder) {
                    ...
                    utils.get('/content/' + item.id + '.json', {}, function (rs) {
                        if (rs.code == 0) {
                            if (self.editing) {
                                plugins[item.type].edit(api, rs.data.content);
                            }
